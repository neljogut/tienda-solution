...
export const Catalog: React.FC = () => {
  ...
  const [showTypeShareMenu, setShowTypeShareMenu] = useState(false);
  const [copiedTypeLink, setCopiedTypeLink] = useState(false);
  ...
  const { getRetailPrice, getCost, productTypes } = usePricingData();
  ...
  const typeUrl = useMemo(() => {
    ...
    return `${window.location.origin}/catalog/type/${selectedType}`;
  }, [selectedType, selectedCategory]);

  const handleShareTypeClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    setShowTypeShareMenu(prev => !prev);
  };

  const handleCopyTypeLink = async (e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      await navigator.clipboard.writeText(typeUrl);
      setCopiedTypeLink(true);
      setTimeout(() => {
        setCopiedTypeLink(false);
        setShowTypeShareMenu(false);
      }, 1500);
    } catch {
      setShowTypeShareMenu(false);
    }
  ...
  const handleShareTypeWhatsApp = (e: React.MouseEvent) => {
    e.stopPropagation();
    const name = productTypes[selectedType] || "Tipo de Producto";
    const text = encodeURIComponent(`Mirá este tipo de productos: ${name}\n${typeUrl}`);
    window.open(`https://wa.me/?text=${text}`, '_blank');
    setShowTypeShareMenu(false);
  };

  const handleShareTypeNative = (e: React.MouseEvent) => {
    e.stopPropagation();
    const name = productTypes[selectedType] || "Tipo de Producto";
    navigator.share({ title: name, url: typeUrl }).catch(() => {});
    setShowTypeShareMenu(false);
  };
  ...
  const handleTypeChange = (newType: string) => {
    if (newType === 'all') {
      ...
      const catTreeIds = selectedCategory !== 'all' ? getCategoryTreeIds(canonicalCategories, selectedCategory) : null;
      const count = catTreeIds ? products.filter(p => {
        const resolved = resolveCategoryId(p.categoryId, idRemap);
        return resolved && catTreeIds.has(resolved) && p.type === newType;
      }).length : 0;

      if (selectedCategory !== 'all' && count > 0) {
        navigate(`/catalog/type/${newType}/category/${selectedCategory}`);
      } else {
        navigate(`/catalog/type/${newType}`);
      }
    }
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="card p-4 bg-white">
        <div className="flex flex-col md:flex-row gap-3 items-stretch md:items-center">
          ...
          {/* Collapsible Types Dropdown Selector */}
          <div className="relative w-full md:w-auto flex items-center gap-2">
            <div className="relative flex-1 md:flex-initial">
              <button
                onClick={() => setShowTypeMenu(!showTypeMenu)}
                className="btn-secondary w-full md:w-48 flex items-center justify-between gap-2 text-xs h-10"
              >
                <span className="truncate">
                  {selectedType === 'all'
                    ? 'Todos los tipos'
                    : productTypes[selectedType] || 'Varios'}
                </span>
                <ChevronDown size={16} className={`transition-transform duration-200 ${showTypeMenu ? 'rotate-180' : ''}`} />
              </button>
              ...
              {showTypeMenu && (
                <>
                  ...
                  />
                  <div className="absolute top-full right-0 md:left-0 mt-2 w-56 bg-white border border-slate-200 shadow-xl rounded-xl p-3 z-30 max-h-80 overflow-y-auto space-y-1 text-xs animate-fadeIn">
                    <button
                    ...
                    >
                      ...
                      <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${
                        selectedType === 'all' ? 'bg-white/20 text-white' : 'bg-slate-200 text-slate-500'
                      }`}>
                        {products.length}
                      </span>
                    </button>
                ...
                {showTypeShareMenu && (
                  <>
                    <div 
                      ...
                      onClick={() => setShowTypeShareMenu(false)} 
                    />
                    <div className="absolute top-full right-0 mt-2 z-50 bg-white rounded-2xl shadow-xl border border-slate-100 p-2 flex flex-col gap-1 min-w-[170px] animate-fadeIn">
                      {/* WhatsApp */}
                      <button
                        onClick={handleShareTypeWhatsApp}
                      ...
                      >
                        {copiedTypeLink ? <CheckIcon size={16} className="text-green-500 flex-shrink-0" /> : <Copy size={16} className="flex-shrink-0" />}
                        <span className="truncate">{copiedTypeLink ? '¡Copiado!' : 'Copiar link'}</span>
                      </button>

                      {/* Native share */}
                      {typeof navigator.share === 'function' && (
                        <button
                          onClick={handleShareTypeNative}
            ...
            <div className="relative flex-1 md:flex-initial">
              ...
              {showCategoryMenu && (
                <>
                  ...
                  />
                  <div className="absolute right-0 md:left-0 mt-2 w-72 bg-white border border-slate-200 shadow-xl rounded-xl p-3 z-30 max-h-80 overflow-y-auto space-y-1 text-xs animate-fadeIn">
                    <button
                    ...
                    >
                      ...
                      <span className={`text-[10px] px-1.5 py-0.2 rounded-full ${
                        ...
                        {selectedType === 'all' ? products.length : products.filter(p => p.type === selectedType).length}
                      </span>
                    </button>
                    ...
                    {visibleCategories.map(cat => {
                      ...
                      const count = products.filter(p => {
                        const resolved = resolveCategoryId(p.categoryId, idRemap);
                        const matchesCategory = resolved && catTreeIds.has(resolved);
